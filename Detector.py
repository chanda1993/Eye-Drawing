...
class CascadeDetector:
    def __init__(self):
        # xml_path = normpath(realpath(cv2.__file__) + 'algorithms')
        # print(xml_path)
        # self.face_cascade = cv2.CascadeClassifier(xml_path + '/haarcascade_frontalface_default.xml')
        self.face_cascade = cv2.CascadeClassifier('algorithms/haarcascade_frontalface_default.xml')
        # self.eye_cascade = cv2.CascadeClassifier(xml_path + '/haarcascade_eye.xml')
        self.eye_cascade = cv2.CascadeClassifier('algorithms/haarcascade_eye.xml')
        detector_params = cv2.SimpleBlobDetector_Params()
        detector_params.filterByArea = True
        detector_params.maxArea = 1500  # 1500
        self.blobDetector = cv2.SimpleBlobDetector_create(detector_params)
        # self.PUPIL_THRESH = cv2.getTrackbarPos('Threshold', 'Eyes Drawing Tool')
    ...
    def detectFace(self, bgr_image):
        ...
        return self.face_cascade.detectMultiScale(gray_image, 1.5, 5)  # 1.3, 5 # TODO: parameters

    # Method for eyes detection
    def detectEyes(self, bgr_image):
        gray_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2GRAY)
        return self.eye_cascade.detectMultiScale(gray_image, 1.5, 5)  # 1.3, 5 # TODO: parameters

    # Method for pupils detection
    def detectPupils(self, bgr_image, threshold=127):
        ...
        # Threshold of values is simple and adaptive thresholding
        _, t_img = cv2.threshold(gray_frame, threshold, 255, cv2.THRESH_TOZERO)
        # t_img = cv2.adaptiveThreshold(gray_frame, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        img = cv2.erode(t_img, None, iterations=2)
        img = cv2.dilate(img, None, iterations=4)
        img = cv2.medianBlur(img, 5)
        return self.blobDetector.detect(img), t_img

    # Method to find eyes after detection
    ...
    # Method to check if eyes are detected
    def check_eyes(self):
    ...
    # Method to check if the face frame is stable
    ...
    # Method to check if the eyes frame are stable
    ...
    # Method to check an overlap area between the face and eyes frame
    ...
    # Method to get the entire images from the webcam
    def get_images(self):
        images = {
    ...
    # Change the sensitivity value (threshold)
    def start_phase(self, phase, threshold=0.90):  # 0.95
        self.phase = phase
